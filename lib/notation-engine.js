...
import {
  ...
  krasuanOf, verseCode, luktokOf, pairId, chingAt, buildVoices,
...
@media (max-width:700px){.thn{--cw:1.72rem}}
...
<div class="thn-bar">
  ...
  <label class="thn-pick" data-t="drumwrap" style="display:none">กลอง <select data-f="nathab"><option value="none">ไม่มี</option><option>ปรบไก่</option><option>สองไม้</option></select>
    <select data-f="drum"><option>ตะโพน</option><option>กลองแขก</option><option>กลองสองหน้า</option><option>โทนรำมะนา</option></select></label>
  <label class="thn-pick">อัตรา <select data-f="level"><option>สามชั้น</option><option selected>สองชั้น</option><option>ชั้นเดียว</option></select></label>
...
<div class="thn-bar">
  ...
  <label class="thn-pick">วรรคละ <select data-f="base"><option value="4" selected>4 ห้อง</option><option value="2">2 ห้อง</option></select></label>
  <label class="thn-pick">บรรทัดละ <select data-f="line"><option value="8" selected>8 ห้อง</option><option value="7">7 ห้อง</option><option value="6">6 ห้อง</option><option value="5">5 ห้อง</option><option value="4">4 ห้อง</option><option value="3">3 ห้อง</option><option value="2">2 ห้อง</option></select></label>
...
export class NotationEngine {
  constructor(root, opts = {}) {
    ...
    this.S = { base: opts.base || 4, lineHong: opts.lineHong || 8, twoHands: !!opts.twoHands, sound: true,
      reg: 0, sabatArm: false, ensemble: opts.ensemble || 'sai', bpm: opts.bpm || 120, chingOn: false,
      level: opts.level || 'สองชั้น', font: 'notation', caret: { v: 0, p: 0, hand: 'r' }, verses: [], total: 0,
  ...
  setVerses(verses) {
    ...
    if (!verses) S.verses = [...this.newLine('ท่อน 1'), ...this.newLine('ท่อน 1')];
      ...
      S.verses = verses.map(v => ({ sec: v.sec, nl: v.nl, cells: v.cells.map(c => ({ r: c.r || [], l: c.l || [] })) }));
  ...
  getVerses() { return trimVerses(this.S.verses).map(v => ({ sec: v.sec, nl: v.nl, cells: v.cells })); }
  getState() { const S = this.S; return { base: S.base, lineHong: S.lineHong, twoHands: S.twoHands, ensemble: S.ensemble, level: S.level }; }
  ...
  destroy() { this.dead = true; clearTimeout(this._an); this.stopPlay(); this.root.innerHTML = ''; this.root.classList.remove('thn', 'fnt-notation', 'fnt-unicode'); document.removeEventListener('keydown', this._key); }
  ...
  syncControls() {
    ...
    this.q('[data-f="base"]').value = String(S.base);
    this.q('[data-f="line"]').value = String(S.lineHong);
    this.q('[data-f="level"]').value = S.level;
    this.q('[data-f="bpm"]').value = S.bpm; this.t('bpm').textContent = S.bpm;
    this.t('split').textContent = S.lineHong + ' ห้อง = ' + splitLine(S.lineHong, S.base).map(h => 'วรรค ' + h).join(' + ');
  }

  /* ─── โครงสร้าง ─── */
  newLine(sec) { return splitLine(this.S.lineHong, this.S.base).map((h, i) => mkVerse(sec, h, i === 0)); }
  ...
  buildRow(row, ctx) {
    ...
    row.vs.forEach((vi, k) => {
      ...
      for (let h = 0; h < hongOf(v); h++) {
        const g = $el('div', 'thn-grp thn-ching');
        for (let p = 0; p < 4; p++) g.append($el('span', '', chingAt(v._off + h * 4 + p, S.level)));
        cbars.append(g);
      }
    });
    ...
    if (lt.some(Boolean)) { const d = $el('span', 'thn-luk'); d.innerHTML = 'ลูกตก <b>' + lt.map(x => x ? x.ch : '–').join(' ') + '</b>'; info.append(d); }
  ...
  rebuild() {
    this.trimTail(); this.reindex(); this.computeRows();
  ...
  async ensureAssets() {
    ...
    if (S.nathab !== 'none' && this.perc && !this.percRows) {
      try { this.percRows = await this.perc.load(); } catch (e) { this.percRows = []; }
    }
  }
  async startPlay(fromStep) {
    ...
    if (S.nathab !== 'none' && this.perc && this.percRows) {
      const row = this.percRows.find(r => r.nathab === S.nathab && r.level === S.level && r.instrument === S.drum);
      if (row) { const p = this.perc.parse(row.pattern_text); drumHits = p.hits; drumLen = p.len; }
    }
    for (let s = fromStep; s < total; s++) {
      const t = t0 + (s - fromStep) * SD; tl.push({ t, s });
      if (S.chingOn) { const c = chingAt(s, S.level); if (c) this.chingHit(t, c === 'ฉับ'); }
      if (drumHits && drumLen > 0) { const pp = (s % drumLen) + 1; drumHits.forEach(h => { if (h.pos === pp) this.perc.play(ctx, h.syll, t, 0.75); }); }
  ...
  relayout() {
    const S = this.S; this.pushUndo();
    const flat = [];
    S.verses.forEach(v => { for (let h = 0; h < hongOf(v); h++) flat.push({ sec: v.sec, cells: v.cells.slice(h * 4, h * 4 + 4) }); });
    while (flat.length && flat[flat.length - 1].cells.every(c => !c.r.length && !c.l.length)) flat.pop();
    const vs = [];
    let i = 0;
    while (i < flat.length) {
      const sec = flat[i].sec;
      const lineHongs = []; while (i < flat.length && lineHongs.length < S.lineHong && flat[i].sec === sec) lineHongs.push(flat[i++]);
      const sizes = splitLine(lineHongs.length, S.base); let at = 0;
      sizes.forEach((sz, k) => { const v = mkVerse(sec, sz, k === 0); for (let h = 0; h < sz; h++) if (lineHongs[at + h]) for (let p = 0; p < 4; p++) v.cells[h * 4 + p] = lineHongs[at + h].cells[p]; at += sz; vs.push(v); });
    }
    S.verses = vs.length ? vs : this.newLine('ท่อน 1');
    S.caret = { v: 0, p: 0, hand: 'r' };
    this.reindex(); this.appendLine(); this.rebuild(); this.emit();
  }
  ...
  bind() {
    ...
    this.q('[data-f="base"]').addEventListener('change', e => { S.base = +e.target.value; this.syncControls(); this.relayout(); });
    this.q('[data-f="line"]').addEventListener('change', e => { S.lineHong = +e.target.value; this.syncControls(); this.relayout(); });
    this.q('[data-f="level"]').addEventListener('change', e => { S.level = e.target.value; this.rebuild(); this.emit(); });
    ...
    this.q('[data-f="drum"]').addEventListener('change', e => { S.drum = e.target.value; });
