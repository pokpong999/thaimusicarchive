...
export class NotationEngine {
  ...
  buildRow(row, ctx) {
    ...
    (S.twoHands ? ['r', 'l'] : ['r']).forEach((hand, hi) => {
      ...
      row.vs.forEach((vi, k) => {
        ...
        const v = S.verses[vi];
        for (let h = 0; h < hongOf(v); h++) {
          const g = $el('div', 'thn-grp cells'); g.dataset.hong = v._off + h * 4;
          for (let p = 0; p < 4; p++) {
  ...
  bind() {
    ...
    this._up = ev => { clearTimeout(this._lp); if (this.drag && this.drag.inSel && !this.drag.moved && !this.move) this.setSel(null); this.drag = null; if (this.move) this.endMove(true); };
    document.addEventListener('mouseup', this._up);
    this._mv = ev => { if (this.move) this.moveAt(ev); };
