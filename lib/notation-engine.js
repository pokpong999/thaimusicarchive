...
<div class="thn-man-grid">
  <section>
    ...
    <table class="thn-man-keys">
      ...
      <tr><td><kbd>Enter</kbd></td><td><b>แทรกห้องว่างทางขวา</b>ของห้องที่เคอร์เซอร์อยู่ แล้วกระโดดเข้าไปพิมพ์ต่อ — ห้องหลังจากนั้นเลื่อนไปข้างหน้า ครบ 8 ห้องตกบรรทัดล่างเอง (ไม่ไหลข้ามท่อน)</td></tr>
      <tr><td><kbd>Ctrl</kbd>+<kbd>Enter</kbd></td><td><b>ลบทั้งห้อง</b>ที่เคอร์เซอร์อยู่ — ห้องถัดไปเลื่อนมาแทน (ห้องที่ขยับเข้ามาจะกะพริบสีทอง · ถ้าห้องรอบ ๆ ว่างเหมือนกันอาจดูไม่ต่าง ดูข้อความบนแถบเครื่องมือ)
        · ใช้ <kbd>Ctrl</kbd>+<kbd>Backspace</kbd> / <kbd>Ctrl</kbd>+<kbd>Delete</kbd> / <kbd>Ctrl</kbd>+<kbd>D</kbd> แทนได้ · Mac ใช้ <kbd>⌘</kbd></td></tr>
...
<div class="thn-bar thn-selbar" data-t="selbar">
  ...
  <button class="thn-btn" type="button" data-a="inshong" title="แทรกห้องว่าง 1 ห้องหน้าห้องที่เคอร์เซอร์อยู่ — ห้องที่เหลือเลื่อนไปข้างหน้า ครบบรรทัดแล้วตกบรรทัดล่างเอง (Enter = แทรกทางขวา)">⇥ แทรกห้อง</button>
  ...
  <button class="thn-btn" type="button" data-a="delhong" title="ตัดห้องที่เคอร์เซอร์อยู่ออก — ห้องที่เหลือเลื่อนมาชิด (Ctrl+Enter)">⇤ ตัดห้องนี้ออก</button>
...
export class NotationEngine {
  ...
  // Enter = แทรกห้องว่างทางขวาของห้องที่เคอร์เซอร์อยู่ แล้วกระโดดเข้าไปพิมพ์ต่อ
  insertBarRight() {
    if (this.readOnly) return;
    const at = this.hongStart() + 4;
    this.insertAt(at, 4);
    ...
    this.flash('แทรกห้องใหม่ทางขวา — ห้องหลังจากนี้เลื่อนไปข้างหน้า (Ctrl+Z คืนได้)');
  }
  // Ctrl+Enter = ตัดห้องที่เคอร์เซอร์อยู่ออกทั้งห้อง
  deleteBar() {
    if (this.readOnly) return;
    const S = this.S, a = this.hongStart(), hn = Math.floor(S.caret.p / 4) + 1, vn = S.caret.v + 1;
    const gone = [0, 1, 2, 3].map(i => { const c = this.cellAt(a + i); const n = c ? (c.r.length ? c.r : c.l) : []; return n.length ? n.map(x => S.font === 'notation' ? noteKey(x) : noteText(x)).join('') : '-'; }).join(' ');
    this.removeRange(a, a + 3);
    // ไฮไลต์ห้องที่ขยับเข้ามาแทน ให้เห็นว่าลบแล้วจริง (ถ้าห้องรอบ ๆ ว่างเหมือนกันจะดูไม่ต่าง)
    const g = this.root.querySelector(`.thn-grp.cells[data-hong="${a}"]`); if (g) { g.classList.add('just'); setTimeout(() => g.classList.remove('just'), 1300); }
    this.flash(`ลบห้อง ${hn} ของ ว.${vn} (${gone}) แล้ว — ห้องถัดไปเลื่อนมาแทน (Ctrl+Z คืนได้)`);
  }
