...
<div class="thn-man-grid">
  <section>
    ...
    <table class="thn-man-keys">
      ...
      <tr><td><kbd>Ctrl</kbd>+<kbd>Enter</kbd></td><td><b>ลบทั้งห้อง</b>ที่เคอร์เซอร์อยู่ — ห้องที่เหลือเลื่อนมาชิด (Mac ใช้ <kbd>⌘</kbd>+<kbd>Enter</kbd>)</td></tr>
...
export class NotationEngine {
  ...
  deleteBar() {
    if (this.readOnly) return;
    const a = this.hongStart(), hn = Math.floor(this.S.caret.p / 4) + 1;
    this.removeRange(a, a + 3);
    this.flash(`ตัดห้อง ${hn} ของ ว.${this.S.caret.v + 1} ออก — ที่เหลือเลื่อนมาชิด (Ctrl+Z คืนได้)`);
  }
  ...
  bind() {
    ...
    this._key = e => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key === 'Enter') { e.preventDefault(); this.deleteBar(); return; }
