  ...
  async startPlay(fromStep) {
    ...
    if (S.nathab !== 'none' && this.perc && this.percRows) {
      ['สามชั้น', 'สองชั้น', 'ชั้นเดียว'].forEach(lv => {
        const row = this.percRows.find(r => r.nathab === S.nathab && r.level === lv && r.instrument === S.drum);
    ...
    for (let s = fromStep; s < total; s++) {
      ...
      if (dp && dp.len > 0) { const pp = (m.rel % dp.len) + 1; dp.hits.forEach(h => { if (h.pos === pp) this.perc.play(ctx, h.syll, t, 0.75); }); }
