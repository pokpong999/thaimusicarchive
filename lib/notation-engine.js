...
export class NotationEngine {
  constructor(root, opts = {}) {
    ...
    this.S = { ta, rap, system: SYSTEMS[opts.system] ? opts.system : (opts.twoHands ? 'hands2' : 'melody1'), sound: true,
      reg: 0, sabatArm: false, bpm: opts.bpm || 120, chingOn: !!opts.chingOn,
  ...
  setVerses(verses) {
    ...
    if (!verses) S.verses = [...this.newLine('ท่อน 1'), ...this.newLine('ท่อน 1')];
      ...
      S.verses = verses.map(v => ({ sec: v.sec, nl: v.nl, level: v.level, ching: v.ching, cells: v.cells.map(c => ({ r: c.r || [], l: c.l || [], x: c.x || [] })) }));
  ...
  copySel() {
    ...
    this.clip = [];
    for (let st = a; st <= b; st++) { const c = this.cellAt(st); this.clip.push(c ? { r: c.r.map(n => ({ ...n })), l: (c.l || []).map(n => ({ ...n })), x: (c.x || []).map(n => ({ ...n })) } : { r: [], l: [], x: [] }); }
  ...
  clearSel() {
    ...
    const [a, b] = this.workRange();
    this.pushUndo();
    for (let st = a; st <= b; st++) { const c = this.cellAt(st); if (c) { c.r = []; c.l = []; c.x = []; } }
    this.rebuild(); this.emit();
  }
  ...
  pasteCells(cells) {
    ...
    cells.forEach((c, i) => { const t = this.cellAt(start + i); if (t) { t.r = (c.r || []).map(n => ({ ...n })); t.l = (c.l || []).map(n => ({ ...n })); t.x = (c.x || []).map(n => ({ ...n })); } });
    this.setSel(null);
    this.rebuild();
  ...
  tone(n, at, gain) {
    ...
    if (this.S.src !== 'synth' && this.audio && this.buffers) {
      const ok = this.audio.play(ctx, this.buffers, n.ch, n.reg, Math.max(ctx.currentTime, at), gain * 2, this.tangShift(), this.srcInst(), { tuning: this.tuningObj(), srcTuning: this.srcTuning(), hzMap: this.hzMap });
