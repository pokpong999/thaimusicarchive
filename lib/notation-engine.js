...
export class NotationEngine {
  ...
  getState() { const S = this.S; return { base: S.base, lineHong: S.lineHong, ta: S.ta, rap: S.rap, system: S.system, lines: lineCount(S.system), twoHands: S.twoHands, ensemble: S.ensemble, tuning: S.tuning, level: S.level, chingMode: S.chingMode }; }
  ...
  hzOfNote(n) { return hzOf(this.tuningObj(), n.ch, n.reg || 0); }
  ...
  tone(n, at, gain) {
    ...
    if (this.S.src !== 'synth' && this.audio && this.buffers) {
      const ok = this.audio.play(ctx, this.buffers, n.ch, n.reg, Math.max(ctx.currentTime, at), gain * 2, 0, this.srcInst(), { tuning: this.tuningObj(), srcTuning: this.srcTuning(), hzMap: this.hzMap });
