...
import {
  NOTES, HIGH, LOW, KEYMAP, KEY_OF, SABAT_GAP_DEFAULT, sabatChain, sabatVel,
...
export const ENGINE_VERSION = '1 ก.ย. 69 · r14';
...
/* ── ธีมของกระดาน (Pk 27 ส.ค. 69) ────────────────────────────────
   thn-dark   กระดานสีเข้ม (ของเดิม)
   thn-paper  กระดาษขาว หมึกดำ — แบบโน้ตที่พิมพ์ใส่กระดาษ
   thn-hicon  ขาว-ดำคมชัดสูง สำหรับฉายโปรเจกเตอร์หรือสายตาไม่ดี
   --thn-rest คือสีของขีด "-" (ตำแหน่งที่ไม่มีเสียง)
              ของเดิมใช้ muted ที่ opacity .55 → contrast แค่ 2.76 มองแทบไม่เห็น
              ตอนนี้แยกเป็นโทเคนของตัวเอง และคุมให้เห็นชัดทุกธีม */
.thn{--thn-bg:var(--navy3,#1E3050);--thn-sunk:var(--navy,#0F1B2D);--thn-raised:var(--navy2,#162336);
  ...
  --thn-alert:var(--danger,#D47A8F);--thn-rest:#9FB3CE;--thn-beat:rgba(201,168,76,.10);
  --thn-hover:rgba(201,168,76,.18);--thn-restw:600;
  --cw:2.15rem;font-family:'Noto Sans Thai',sans-serif;color:var(--thn-ink)}

.thn.thn-paper{--thn-bg:#FBFAF6;--thn-sunk:#FFFFFF;--thn-raised:#F3F1E9;
  --thn-ink:#12161C;--thn-muted:#5F6670;--thn-line:#C6CBD3;
  --thn-gold:#8A6D14;--thn-gold2:#6E5510;--thn-jade:#1F6B57;--thn-alert:#A3312B;
  --thn-rest:#5F6670;--thn-beat:rgba(138,109,20,.10);--thn-hover:rgba(138,109,20,.14);--thn-restw:500}

.thn.thn-hicon{--thn-bg:#FFFFFF;--thn-sunk:#FFFFFF;--thn-raised:#FFFFFF;
  --thn-ink:#000000;--thn-muted:#333333;--thn-line:#000000;
  --thn-gold:#7A5C00;--thn-gold2:#5C4500;--thn-jade:#00563F;--thn-alert:#8E1B14;
  --thn-rest:#333333;--thn-beat:rgba(0,0,0,.09);--thn-hover:rgba(0,0,0,.14);--thn-restw:700}
.thn.thn-hicon .thn-score,.thn.thn-hicon .thn-bar,.thn.thn-hicon .thn-panel{border-width:2px}
.thn.thn-hicon .thn-vsep{background:#000}

/* ธีมสว่างต้องพลิกสีข้อความบนปุ่มเขียว/ทองให้อ่านออก */
.thn.thn-paper .thn-btn.play,.thn.thn-hicon .thn-btn.play{color:#fff}
.thn.thn-paper .thn-seg button[aria-pressed="true"],
.thn.thn-hicon .thn-seg button[aria-pressed="true"]{color:#fff}
.thn.thn-paper .thn-nkey,.thn.thn-hicon .thn-nkey{background:#fff}
...
.thn-cell.beat{background:var(--thn-beat)}
/* ขีด "-" ต้องเห็นชัด แต่ยังเบากว่าตัวโน้ตจริง (Pk 27 ส.ค. 69: "มันกลืนไปหมด")
   เดิม muted ที่ .55 → contrast 2.76 · ตอนนี้ 8.08 บนกระดานสีเข้ม */
.thn-cell.empty .gl{color:var(--thn-rest);opacity:1;font-weight:var(--thn-restw)}
.thn-cell.lead .gl{color:var(--thn-jade)}
.thn-cell:hover{background:var(--thn-hover)}
...
@media (max-width:820px){
  ...
  /* มือถือ: แป้นโน้ตลอยล่างจอกินพื้นที่ไปกว่า 1 ใน 3 ของหน้าจอ (308px จาก 844px)
     ตัวการคือคำอธิบายคีย์ลัดคีย์บอร์ด ซึ่งบนโทรศัพท์ไม่มี Ctrl/Alt/Shift ให้กดอยู่แล้ว
     → ซ่อนคำอธิบายนั้น และคุมไม่ให้แป้นสูงเกินครึ่งจอ (Pk 27 ส.ค. 69) */
  .thn-help{display:none}
  .thn-pad{max-height:46vh;overflow-y:auto;padding:.5rem;gap:.4rem}
  .thn-nkey{min-width:2.5rem;height:2.5rem}
  .thn-pad .thn-btn{padding:8px 10px}}
...
<div class="thn-bar">
  ...
  <label class="thn-pick" title="สีของกระดาน — เลือกกระดาษขาว-ดำได้ถ้าอ่านง่ายกว่า"><select data-f="theme">
    <option value="dark">🌙 กระดานสีเข้ม</option>
    <option value="paper">📄 กระดาษขาว-ดำ</option>
    <option value="hicon">◑ ขาว-ดำคมชัดสูง</option></select></label>
...
export class NotationEngine {
  ...
  setVerses(verses) {
    ...
    this.reindex();
    this.setTheme(NotationEngine.readTheme(), { remember: false });
    this.syncControls();
    this.rebuild();
  }
  ...
  // ── ธีมกระดาน ────────────────────────────────────────────────
  static themeKey = 'thma-board-theme';
  static readTheme() {
    try { const v = localStorage.getItem('thma-board-theme'); return ['dark','paper','hicon'].includes(v) ? v : 'dark'; }
    catch (e) { return 'dark'; }
  }
  setTheme(name, { remember = true } = {}) {
    const v = ['dark', 'paper', 'hicon'].includes(name) ? name : 'dark';
    this.root.classList.remove('thn-paper', 'thn-hicon');
    if (v !== 'dark') this.root.classList.add('thn-' + v);
    const sel = this.q('[data-f="theme"]'); if (sel) sel.value = v;
    this.S.theme = v;
    if (remember) { try { localStorage.setItem('thma-board-theme', v); } catch (e) {} }
    return v;
  }
  ...
  async startPlay(fromStep) {
    ...
    for (let s = fromStep; s < total; s++) {
      ...
      const lead = consumed.length > 1
        ...
        : [];
      // สะบัดที่คร่อมสองมือ: รวมทุกแนวเป็นชุดเดียว เว้นระยะเท่ากัน (Pk 27 ส.ค. 69)
      // ใช้กฎเดียวกับเครื่องเล่นหน้าเพลง กระดานกับหน้าเพลงจะได้ฟังเหมือนกัน
      const lanes = Array.from({ length: consumed.length }, (_, li) =>
        Array.from({ length: total }, (_, k) => at(li, k)));
      const chain = sabatChain(lanes, runs, consumed, s, consumed.length, total);
      if (chain) {
        chain.forEach((c, i) => {
          const back = chain.length - 1 - i;
          const damp = consumed.length > 1 ? isDamp(cellHere, HKp[c.lane]) : !!dampMask(cellHere);
          this.tone(c.note, t - back * SABAT_GAP_DEFAULT, 0.42 * sabatVel(back), { damp });
        });
        continue;
      }
      for (let li = 0; li < consumed.length; li++) {   // ทุกบรรทัดตามระบบบันทึก (ขิม/จะเข้ = 3)
        ...
        at(li, s).forEach(n => this.tone(n, tt, 0.42, { damp }));
      }
    }
  ...
  clearCurCell() {
    ...
    // ★ ลบโน้ต = ลบเครื่องหมายของโน้ตนั้นไปด้วย (Backspace/Delete) — Pk 1 ก.ย. 69 "ลบกรอไม่ได้"
    //   กรอ: คลื่นอยู่เหนือแนวบน (แนวแรกของระบบ) — ลบโน้ตแนวบนเมื่อไหร่ กรอหายทันที
    //        แม้แนวล่างจะยังมีโน้ตอยู่ (ของเดิมรอให้ "ทั้งช่อง" ว่างก่อน กรอจึงค้างในโน้ตสองมือ)
    //   ประคบ: เป็นรายมือ ลบโน้ตมือไหน ก็ถอดประคบของมือนั้น
    const top = this.hands()[0] || 'r';
    if (cell.m === 'kro' && hk === top) delete cell.m;
    if (cell.d) setDamp(cell, hk, false);
    if (!(cell.r || []).length && !(cell.l || []).length && !(cell.x || []).length) {
      delete cell.m; delete cell.d;
    }
  }
  backspace() {
    if (this.readOnly) return;
  ...
  bind() {
    ...
    // ธีมกระดาน — จำไว้ในเครื่องผู้ใช้ ใช้ต่อทุกครั้งที่เปิดกระดาน (Pk 27 ส.ค. 69)
    { const th = this.q('[data-f="theme"]'); if (th) th.addEventListener('change', e => this.setTheme(e.target.value)); }
