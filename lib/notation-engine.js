...
import { SYSTEMS, SYSTEM_KEYS, systemOf, linesOf, handsOf, lineCount, systemForLines } from './notation-systems';
...
export class NotationEngine {
  ...
  getState() { const S = this.S; return { base: S.base, lineHong: S.lineHong, ta: S.ta, rap: S.rap, system: S.system, lines: lineCount(S.system), twoHands: S.twoHands, ensemble: S.ensemble, level: S.level, chingMode: S.chingMode }; }
  ...
  tone(n, at, gain) {
    ...
    if (this.S.src !== 'synth' && this.audio && this.buffers) {
      const ok = this.audio.play(ctx, this.buffers, n.ch, n.reg, Math.max(ctx.currentTime, at), gain * 2, this.S.ensemble === 'piphat' ? 1 : 0, this.srcInst());
  ...
  bind() {
    ...
    { const sy = r.querySelector('[data-f="system"]');
      if (sy) sy.addEventListener('change', () => { S.system = SYSTEMS[sy.value] ? sy.value : 'melody1'; if (!handsOf(S.system).includes(S.caret.hand)) S.caret.hand = 'r'; this.ensureHands(); this.syncControls(); this.rebuild(); this.emit(); }); }
    ...
    this.q('[data-f="src"]').addEventListener('change', e => { S.src = e.target.value; this.buffers = null; if (S.src !== 'synth') this.ensureAssets(); this.emit(); });
    ...
    if (S.src !== 'synth') this.ensureAssets();   // โหลดเสียงจริงตั้งแต่เปิดกระดาน ให้เสียงขณะพิมพ์เป็นเสียงเครื่องจริงด้วย
