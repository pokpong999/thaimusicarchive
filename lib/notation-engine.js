...
<div class="thn-man-grid">
  ...
  <section>
    ...
    ใช้แก้โน้ตที่ถอดจาก PDF แล้วหายไปสองสามห้องจนรวนทั้งเพลง</p>
...
<div class="thn-bar thn-selbar" data-t="selbar">
  ...
  <button class="thn-btn" type="button" data-a="inshong" title="แทรกห้องว่าง 1 ห้องหน้าห้องที่เคอร์เซอร์อยู่ — ห้องที่เหลือเลื่อนไปข้างหน้า ครบบรรทัดแล้วตกบรรทัดล่างเอง">⇥ แทรกห้อง</button>
  ...
  <button class="thn-btn" type="button" data-a="delhong" title="ตัดห้องที่เคอร์เซอร์อยู่ออก — ห้องที่เหลือเลื่อนมาชิด">⇤ ตัดห้องนี้ออก</button>
...
export class NotationEngine {
        ...
        for (let h = 0; h < hongOf(v); h++) {
          ...
          for (let p = 0; p < 4; p++) {
            ...
            c.addEventListener('mousedown', ev => {
              ...
              else { this.setSel(null); this.setCaret({ v: vi, p: idx, hand }); if (!this.readOnly) this.drag = { anchor: step }; }
              if (this.play.on) this.startPlay(step);
              this.t('score').focus();
            });
            c.addEventListener('mouseenter', () => { if (this.drag && this.drag.anchor !== step) this.setSel(this.drag.anchor, step); });
  ...
  bind() {
    ...
    this._key = e => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'c') { e.preventDefault(); this.copySel(); return; }
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'x') { e.preventDefault(); this.cutSel(); return; }
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'v') { if (this.clip) { e.preventDefault(); this.pasteCells(this.clip); } return; }   // ไม่มีคลิปภายใน → ปล่อยให้ paste event อ่านข้อความ
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'a') { e.preventDefault(); this.setSel(0, S.total - 1); return; }
