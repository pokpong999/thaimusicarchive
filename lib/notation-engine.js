...
<section class="thn-panel" style="margin-top:.6rem"><p class="thn-plabel">วางโน้ตที่มีอยู่แล้ว</p>
...
export class NotationEngine {
  ...
  startPlay(fromStep) {
    ...
    const tick = () => {
      ...
      if (cur !== this.play.lastStep) { this.clearPlayMark(); const m = this.cellEls[cur]; if (m) { if (m.r) m.r.classList.add('play'); if (m.l) m.l.classList.add('play'); if (m.r) m.r.scrollIntoView({ block: 'nearest', inline: 'nearest' }); } this.play.lastStep = cur; }
  ...
  skip() { this.pushUndo(); this.curCell()[this.handKey()] = []; this.touchCaretRow(); this.advance(); this.emit(); }
  backspace() {
