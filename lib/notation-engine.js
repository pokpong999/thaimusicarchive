...
export const ENGINE_VERSION = '26 ส.ค. 69 · r8';
...
.thn-newsep{display:inline-block;margin-top:.35rem;padding:0 .35rem;border:1px dashed var(--thn-line);border-radius:4px;color:var(--thn-gold2);
  font-family:monospace;font-size:.8rem;line-height:1.3;cursor:grab;user-select:none}
.thn-newsep:hover{border-color:var(--thn-gold2);background:rgba(232,201,106,.12)}
.thn.readonly .thn-newsep{display:none}
.thn-sec-tools{display:flex;gap:3px;align-items:center;flex-wrap:wrap}
.thn-sec-tools button{background:transparent;border:1px solid transparent;color:var(--thn-muted);font:inherit;font-size:.7rem;
  padding:1px 6px;border-radius:4px;cursor:pointer;white-space:nowrap;line-height:1.5}
.thn-sec-tools button:hover{border-color:var(--thn-gold);color:var(--thn-gold)}
.thn-sec-tools button[disabled]{opacity:.3;cursor:default}
...
/* เครื่องหมายวิธีบรรเลง (Pk 2026-08-26): ประคบ = ตัวหนา · กรอ = คลื่นเหนือโน้ต ยาวถึงเสียงถัดไป */
.thn-cell.damp .gl{font-weight:800;text-shadow:0 0 1px currentColor}
.thn-cell.damp::before{content:'';position:absolute;left:50%;bottom:2px;width:3px;height:3px;margin-left:-1.5px;
  border-radius:50%;background:var(--thn-gold2);opacity:.9}
...
<div class="thn-man-grid">
  ...
  <section>
    ...
    <table class="thn-man-keys">
      ...
      <tr><td><kbd>Alt</kbd>+<kbd>Shift</kbd>+โน้ต</td><td>ลงโน้ตพร้อม <b>ประคบ</b></td></tr>
      <tr><td><kbd>~</kbd> / <kbd>.</kbd></td><td>สลับ กรอ / ประคบ ที่ช่องที่เคอร์เซอร์อยู่ (ไว้คลิกกลับมาแก้ทีหลัง)</td></tr>
    ...
    <p>บนตาราง <b>กรอ</b> ขึ้นเป็นเส้นคลื่นเหนือโน้ต ลากยาวไปจนถึงเสียงถัดไป — นั่นคือช่วงที่จะกรอจริงตอนเล่น ·
    <b>ประคบ</b> ขึ้นเป็นตัวหนามีจุดใต้ตัวโน้ต</p>
...
<div class="thn-pad">
  ...
  <button class="thn-btn" type="button" data-a="damp" style="height:2.7rem" title="ประคบ — มืออีกข้างกดให้เสียงสั้น (Alt+Shift+โน้ต · แป้น . สลับที่ช่องเคอร์เซอร์)">●</button>
...
export class NotationEngine {
  ...
  setVerses(verses) {
    ...
    if (!verses) S.verses = [...this.newLine('ท่อน 1'), ...this.newLine('ท่อน 1')];
      ...
      S.verses = verses.map(v => ({ sec: v.sec, nl: v.nl, level: v.level, ching: v.ching, cells: v.cells.map(c => (c.m ? { r: c.r || [], l: c.l || [], x: c.x || [], m: c.m } : { r: c.r || [], l: c.l || [], x: c.x || [] })) }));
  ...
  deleteSection(vi) {
    ...
    if (S.verses.slice(b.from, b.to + 1).some(hasSound)
        && !window.confirm(`ท่อน "${b.sec}" มีโน้ตอยู่ ${n} วรรค — ลบทั้งท่อน?`)) return;
      ...
      row.vs.forEach((vi, k) => {
        ...
        for (let h = 0; h < hongOf(v); h++) {
          ...
          for (let p = 0; p < 4; p++) {
            ...
            if (cell.m === 'damp') c.classList.add('damp');
  ...
  // คลื่นกรอ: ลากจากช่องที่ติดเครื่องหมาย ไปจนสุดช่องก่อนเสียงถัดไป
  drawKro(wrap) {
    (wrap._kroJobs || []).forEach(({ hr, step }) => {
      const HK = this.hands();
      const from = this.cellEls[step] && this.cellEls[step][HK[0] || 'r'];
      if (!from) return;
      const end = this.nextSoundStep(step);
      let to = from;
      for (let k = end - 1; k > step; k--) {
        const el = this.cellEls[k] && this.cellEls[k][HK[0] || 'r'];
        if (el && el.closest('.thn-hrow') === hr) { to = el; break; }   // ไม่ลากข้ามบรรทัด
      }
      const box = hr.getBoundingClientRect(), a = from.getBoundingClientRect(), b = to.getBoundingClientRect();
      const w = Math.max(10, b.right - a.left - 4);
    ...
    this.rows.forEach((row, ri) => {
      if (row.newSec) {
        ...
        if (!this.readOnly) {
          ...
          mk('🗑', 'ลบท่อนนี้ทั้งท่อน', () => this.deleteSection(at), blocks.length <= 1);
          head.append(tools);
        }
  ...
  copySel() {
    ...
    this.clip = [];
    for (let st = a; st <= b; st++) { const c = this.cellAt(st); this.clip.push(c ? { r: c.r.map(n => ({ ...n })), l: (c.l || []).map(n => ({ ...n })), x: (c.x || []).map(n => ({ ...n })), m: c.m } : { r: [], l: [], x: [] }); }
  ...
  clearSel() {
    ...
    const [a, b] = this.workRange();
    this.pushUndo();
    for (let st = a; st <= b; st++) { const c = this.cellAt(st); if (c) { c.r = []; c.l = []; c.x = []; delete c.m; } }
    this.rebuild(); this.emit();
  }
  ...
  pasteCells(cells) {
    ...
    cells.forEach((c, i) => { const t = this.cellAt(start + i); if (t) { t.r = (c.r || []).map(n => ({ ...n })); t.l = (c.l || []).map(n => ({ ...n })); t.x = (c.x || []).map(n => ({ ...n })); if (c.m) t.m = c.m; else delete t.m; } });
    this.setSel(null);
    this.rebuild();
  ...
  async startPlay(fromStep) {
    ...
    for (let s = fromStep; s < total; s++) {
      ...
      const damp = (this.cellAt(s) || {}).m === 'damp';
      for (let li = 0; li < consumed.length; li++) {   // ทุกบรรทัดตามระบบบันทึก (ขิม/จะเข้ = 3)
        ...
        if (run) run.notes.forEach((n, i) => { const back = run.notes.length - 1 - i; this.tone(n, t - back * SABAT_GAP_DEFAULT, back === 0 ? 0.42 : back === 1 ? 0.34 : 0.28, { damp }); });
        else at(li, s).forEach(n => this.tone(n, t, 0.42, { damp }));
      }
    }
  ...
  // สลับเครื่องหมายที่ช่องที่เคอร์เซอร์อยู่ (แป้น ~ = กรอ · . = ประคบ) — ไว้กลับมาแก้ทีหลัง
  toggleMark(mark) {
    if (this.readOnly) return;
    const cell = this.curCell();
    this.pushUndo();
    const was = cell.m || '';
    if (was === mark) delete cell.m; else cell.m = mark;
  ...
  bind() {
    ...
    this._key = e => {
      ...
      // ~ สลับกรอ · . สลับประคบ ที่ช่องที่เคอร์เซอร์อยู่
      if (k === '~' || k === '`' || code === 'backquote') { e.preventDefault(); this.toggleMark('kro'); return; }
      if (k === '.' || code === 'period') { e.preventDefault(); this.toggleMark('damp'); return; }
