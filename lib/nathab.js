...
export const mainRule = rules => (rules ?? []).find(r => !r.section) ?? null;

/* ── ค่าเริ่มต้นของเพลง: ฉิ่ง / กลอง / ความเร็ว (Pk 2026-08-26) ──
   เก็บที่แถวหน้าทับหลักของ song_nathab (section = null) คอลัมน์ ching / bpm — ต้องรัน sql/18 ก่อน
   บันทึกจากกระดานตอนกดบันทึกโน้ต หรือจากแผง 🥁 หน้าทับ ในหน้าเพลง
   สิทธิ์บังคับจริงด้วย RLS ของ song_nathab (แอดมิน หรือเจ้าของเพลง)                          */
export async function saveSongDefaults(songId, { nathab, level, drum, ching, bpm } = {}) {
  if (!songId) return { error: null, skipped: true };
  const main = mainRule(await loadSongNathab(songId));
  const pick = (nathab && nathab !== 'none') ? nathab : (main?.nathab ?? null);
  const row = {
    song_id: songId, section: null, scope: 'only',
    nathab: pick, level: level || main?.level || null, drum: drum || main?.drum || null,
    ching: !!ching,
    bpm: bpm ? Math.max(30, Math.min(300, Math.round(bpm))) : null,
  };
  // ไม่มีอะไรให้จำเลย (ไม่มีหน้าทับ ไม่เปิดฉิ่ง ไม่ตั้งความเร็ว) → ไม่ต้องสร้างแถวเปล่า
  if (!row.nathab && !row.ching && !row.bpm) return { error: null, skipped: true };
  if (!row.nathab) row.nathab = '-';                 // คอลัมน์ nathab เป็น not null ในฐานเดิม
  const q = main
    ? await supabase.from('song_nathab').update(row).eq('id', main.id)
    : await supabase.from('song_nathab').insert(row);
  return { error: q.error ?? null, skipped: false };
}
