...
/* ข้อยกเว้นที่ Pk เคาะ (27 ส.ค. 69):
     ชั้นเดียว  — ไม่ทอดไม่ถอนชัดเจน จังหวะเร็วต่อเนื่องกันไป
     ลูกหมด    — ไม่มีทอดไม่มีถอน เร่งขึ้นตั้งแต่ต้นลูกหมดจนถึงเสียงสุดท้าย
   ทั้งสองกรณีจึง "เดินต่อเนื่อง": รับความเร็วท้ายท่อนก่อนหน้ามาเป็นจุดเริ่ม ไม่ตกกลับไปที่ความเร็วตั้งต้น
   และไม่ผ่อนท้ายท่อน แม้ผู้ใช้จะติ๊กถอน/ทอดไว้ก็ไม่มีผล                                        */
export const isContinuousSection = ({ level, name } = {}) =>
  level === 'ชั้นเดียว' || /ชั้นเดียว/.test(name ?? '') || /ลูกหมด/.test(name ?? '');
export const CONTINUOUS_WHY = ({ level, name } = {}) =>
  (/ลูกหมด/.test(name ?? '') ? 'ลูกหมด' : (level === 'ชั้นเดียว' || /ชั้นเดียว/.test(name ?? '')) ? 'ชั้นเดียว' : '');
export const MAX_FACTOR = 3;                 // กันเร่งจนหลุดโลกในเพลงยาว ๆ
...
export function tempoPlan({ seq, blockOf, modeOf, halfCycleOf, continuousOf, base = 120, opts = {} }) {
  ...
  let dictated = 1;        // จุดเริ่มของรอบถัดไป "ตามกฎ" ของรอบก่อน (ปกติ = กลับไปความเร็วตั้งต้น)
  let prevEnd = 1;         // ความเร็วท้ายรอบก่อน (ก่อนผ่อนทอด) — ไว้ให้ท่อนต่อเนื่องรับช่วงต่อ
  let prevMode = '';
  passes.forEach(p => {
    const cont = continuousOf ? !!continuousOf(p.block) : false;
    const mode = cont ? '' : (modeOf(p.block) || '');      // ชั้นเดียว/ลูกหมด ไม่ถอนไม่ทอด
    // ท่อนต่อเนื่องรับความเร็วท้ายท่อนก่อนหน้ามาเลย ยกเว้นท่อนก่อนหน้าสั่งไว้เอง (ถอน/ทอด) ก็เคารพคำสั่งนั้น
    const startFactor = (cont && !prevMode) ? prevEnd : dictated;
    const len = p.to - p.from + 1;
    const endFactor = Math.min(MAX_FACTOR, startFactor * (1 + (mode === 'thon' ? accelThon : accel)));
    for (let i = 0; i < len; i++) {
    ...
    if (mode === 'thot') {
      ...
      dictated = 1;                                           // ท่อนถัดไปกลับมาความเร็วปกติ
    } else if (mode === 'thon') {
      dictated = endFactor * thonRatio;                       // ท่อนถัดไปเริ่มจากความเร็วท้ายท่อนนี้
    } else if (cont) {
      dictated = endFactor;                                   // เดินต่อเนื่อง ไม่ตกกลับไปความเร็วตั้งต้น
    } else {
      dictated = 1;
    }
    prevEnd = endFactor; prevMode = mode;
  });
