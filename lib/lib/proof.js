...
export const PROOF = [
  { v: 'none',  label: 'ยังไม่ตรวจ',   en: 'Not checked', icon: '⬜', color: 'var(--muted)'  },
  { v: 'doing', label: 'กำลังตรวจ',    en: 'Checking',    icon: '🔍', color: 'var(--gold2)'  },
  { v: 'ok',    label: 'ตรวจแล้วถูก',  en: 'Verified',    icon: '✅', color: 'var(--jade)'   },
  { v: 'bad',   label: 'พบที่ผิด',     en: 'Error found', icon: '⚠️', color: 'var(--danger)' },
];
// ป้ายตามภาษาที่ผู้ใช้เลือก — ส่ง lang จาก useLang() เข้ามา
export const proofLabel = (p, lang) => (lang === 'en' ? (p.en || p.label) : p.label);
...
export function proofWho(song, names = {}, lang = 'th') {
  if (!song?.proof_at) return '';
  const who = names[song.proof_by] ?? (lang === 'en' ? 'staff' : 'ผู้ดูแล');
  const d = new Date(song.proof_at);
  const when = isNaN(d) ? '' : d.toLocaleDateString(lang === 'en' ? 'en-GB' : 'th-TH',
