...
// แปลข้อความ error ให้อ่านรู้เรื่อง — แต่ห้ามกลืนสาเหตุจริง
//   บทเรียน 27 ส.ค. 69: เดิมจับคำว่า "relation" กับ "function" กว้างเกินไป
//   จน error จริง (infinite recursion detected in policy for relation "classrooms")
//   ถูกแปลผิดเป็น "ยังไม่ได้รัน sql" — ไล่บั๊กผิดทางไปพักใหญ่
export const READY_MSG = 'ยังไม่ได้เปิดระบบห้องเรียนในฐานข้อมูล — ผู้ดูแลต้องรัน sql/25 · sql/26 · sql/27 ก่อน';
const RECURSION = 'กฎสิทธิ์ของห้องเรียนวนกันเอง — ผู้ดูแลต้องรัน sql/27_classroom_rls_fix.sql';
export function friendly(e) {
  const m = e?.message ?? '';
  if (!m) return '';
  if (/infinite recursion/i.test(m)) return RECURSION + `\n(${m})`;
  // "ยังไม่ได้สร้างตาราง" จริง ๆ มีรูปแบบเดียว: relation/table ... does not exist
  if (/does not exist/i.test(m) || /schema cache/i.test(m)) return READY_MSG + `\n(${m})`;
  return m;   // อย่างอื่นแสดงตามจริง จะได้ไล่ถูกจุด
}
