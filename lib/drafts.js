...
export async function saveDraft({ id = null, kind, title, payload }) {
  // updated_at ใช้เรียงลำดับและแสดง "แก้ล่าสุด" — ต้องเซ็ตเอง ไม่งั้นค้างที่เวลาสร้าง
  const row = { kind, title: (title || '').slice(0, 200) || null, payload: payload ?? {}, updated_at: new Date().toISOString() };
  if (id) {
    const { error } = await supabase.from('drafts').update(row).eq('id', id);
    return { id, error: error ?? null };
  }
  const { data: au } = await supabase.auth.getUser();
  const { data, error } = await supabase.from('drafts').insert({ ...row, user_id: au?.user?.id ?? null }).select().single();
