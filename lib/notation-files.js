...
import { pdfItemsToLines } from './notation-import';
...
async function readPdf(file, { onProgress, maxPages = 10 } = {}) {
  ...
  const lines = []; let notationChars = 0; let fontHint = null;
  for (let i = 1; i <= n; i++) {
    ...
    const pl = pdfItemsToLines(items);
...
async function readDocx(file) {
  ...
  walk(body);
  return { kind: 'text', text: lines.join('\n'), name: file.name, fontHint, source: 'docx' };
}
