...
export function baseName(section) {
  ...
  // ตัวคั่นที่ค้างอยู่หลังตัดคำว่า 'ท่อน N' ออก
  //   'คลื่นกระทบฝั่ง สองชั้น – ท่อน 1' → 'คลื่นกระทบฝั่ง สองชั้น –' → ต้องเหลือแค่ชื่อเพลง
...
// ── ๘ · ตรวจ/ซ่อมลำดับวรรคของเพลงเรื่อง (sql/38) ──────────────────
//   อาการที่ต้องซ่อม: บันทึกจากหน้าเพลงย่อยรุ่นเก่า แล้วเลขวรรคของเพลงย่อย
//   ไปทับเลขวรรคของเพลงเรื่อง ทำให้เรียงแล้วท่อนสลับกันมั่ว (Pk 28 ส.ค. 69)
//   รุ่นนี้แก้ต้นเหตุแล้ว ปุ่มนี้ไว้ซ่อมของที่พังไปก่อนหน้า
export async function suiteCheck(songId) {
  const { data, error } = await supabase.rpc('thma_suite_check', { p_song: songId });
  if (error) return { rows: null, error };
  return { rows: data ?? [], error: null };
}

// ข้อความบอกสาเหตุแบบที่คนอ่านรู้เรื่อง — เดิมพลาดตรงนี้แล้วหน้าเว็บเงียบสนิท
export function checkError(m = '') {
  if (/thma_suite_check|thma_suite_order|thma_suite_peek|schema cache|does not exist/i.test(m))
    return 'ยังไม่ได้รัน sql/40_suite_guard.sql (หรือ Supabase ยังไม่รีเฟรช) · ' + m;
  return m;
}

// ★ ส่องลำดับจริงในฐาน — เขียนขึ้นเพราะเดากันมาหลายรอบแล้วยังไม่ตรง
//   ต้องเห็นของจริงว่าแถวเรียงยังไง เลขวรรคเท่าไหร่ อยู่เพลงย่อยไหน
export async function suitePeek(songId, limit = 14) {
  const { data, error } = await supabase.rpc('thma_suite_peek', { p_song: songId, p_limit: limit });
  if (error) return { rows: null, error };
  return { rows: data ?? [], error: null };
}

export async function fixSuiteVerses(songId) {
  const { data, error } = await supabase.rpc('thma_fix_suite_verses', { p_song: songId });
  if (error) return { rows: [], error };
  const { error: e2 } = await supabase.rpc('thma_renumber_lines', { p_song: songId });
  return { rows: data ?? [], error: e2 ?? null };
}
