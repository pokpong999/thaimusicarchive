...
import { sentenceRangeLabel } from './staff';
...
export function splitLines(lines, maxHongs = MAX_HONGS_PER_LINE) {
  ...
  return out;
  // ป้ายแบบเดียวกับบนจอ: "ประโยคที่ 2 · ลูกตก ซ / ซฺ" (๒ วรรค = ๑ ประโยค นับใหม่ทุกท่อน — Pk 2 ก.ย. 69 ดึก)
  function mkLabel(vs) {
    const luk = vs.map(v => v.luktok).filter(Boolean).join(' / ');
    const ents = vs.map(v => v.sn).filter(Boolean);
    const base = ents.length
      ? sentenceRangeLabel(ents, { cont: ents[0].half === 1, showSec: false })
      // แถวเก่าที่ไม่มีเลขประโยคติดมา (ชีตจากรุ่นก่อน) — เดาแบบ ๒ วรรค = ๑ ประโยค จากเลขวรรค
      : (() => { const no = v => (v.label != null ? v.label : v.vi + 1);
                 const a = Math.ceil(no(vs[0]) / 2), b = Math.ceil(no(vs[vs.length - 1]) / 2);
                 return 'ประโยคที่ ' + (b > a ? `${a}–${b}` : a); })();
    return base + (luk ? ` · ลูกตก ${luk}` : '');
  }
}
...
/* ── โน้ตสากล: จัดบรรทัด SVG ของ StaffNotation ลงกรอบวิดีโอ (Pk 1 ก.ย. 69 ข้อ 3) ──
   rows = จาก StaffNotation.onRows: [{ svg, width, height, rects:[{x,w,start,size}], verses:[{verseIdx,offset,len}], label }]
   ภาพแต่ละบรรทัดถูกย่อ/ขยายเท่ากันทั้งแผ่นให้บรรทัดที่กว้างสุดพอดีจอ (ไม่เกิน 2.4 เท่า)
   ไฟวิ่ง = แถบครอบห้องที่กำลังเล่น + เส้นวิ่งตามตำแหน่ง (แบบเดียวกับบนเว็บ)               */
export const STAFF_BAND = { top: 72, height: 72 };   // แถบไฮไลต์คร่อมเส้นบรรทัดทั้ง 5 (VexFlow วาดเส้นแรกที่ STAVE_Y+40) ตาม StaffNotation
export function layoutStaffSheet(rows, { W = VIDEO_W, H = VIDEO_H, padX = 72 } = {}) {
  const list = (rows || []).filter(r => r && r.width > 0);
  const maxW = Math.max(1, ...list.map(r => r.width));
  const scale = Math.min(2.4, (W - padX * 2) / maxW);
  const labelH = 30, gap = 14;
  let y = 0;
  const lines = list.map((r, i) => {
    const y0 = y; y += labelH;
    const notesY = y, imgH = Math.round(r.height * scale); y += imgH + gap;
    return { i, y: y0, h: y - y0, notesY, imgH, scale, width: Math.round(r.width * scale),
      label: r.label || '', img: r.img || null, svg: r.svg || null,
      rects: (r.rects || []).map(rc => ({ x: padX + rc.x * scale, w: rc.w * scale, start: rc.start, size: rc.size })),
      verses: r.verses || [] };
  });
  const index = new Map();
  lines.forEach(ln => ln.verses.forEach(v => { if (!index.has(v.verseIdx)) index.set(v.verseIdx, ln.i); }));
  return { W, H, padX, lines, totalH: y, index, staff: true, rowsMax: 1, fontNote: 40, scale };
}
// ห้อง (rect) + สัดส่วนในห้อง ของเคอร์เซอร์ตัวหนึ่งบนแผ่นโน้ตสากล
export function staffCursorRect(L, cur) {
  if (!cur) return null;
  // r6: วรรคข้ามบรรทัดได้ — แต่ละบรรทัดถือ "ช่วง" ของวรรค (posStart..posStart+len) ต้องไล่หาบรรทัดที่ช่วงครอบ pos
  for (const ln of L.lines) {
    const vm = ln.verses.find(v => v.verseIdx === cur.verseIdx
      && cur.pos >= (v.posStart ?? 0) && cur.pos < (v.posStart ?? 0) + v.len);
    if (!vm) continue;
    const gpos = vm.offset + cur.pos - (vm.posStart ?? 0);
    const rect = ln.rects.find(r => gpos >= r.start && gpos < r.start + r.size); if (!rect) continue;
    return { line: ln, rect, frac: (gpos - rect.start + 0.5) / rect.size };
  }
  return null;
}
// แปลง SVG ของแต่ละบรรทัดเป็นรูป (ทาสีหมึกตามธีมก่อน) — ต้องรอโหลดครบก่อนเริ่มอัด
export async function prepareStaffImages(rows, ink) {
  const out = [];
  for (const r of rows || []) {
    if (!r.svg) { out.push({ ...r, img: null }); continue; }
    const c = r.svg.cloneNode(true);
    c.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
    c.querySelectorAll('*').forEach(el => {
      const f = el.getAttribute('fill'); if (f && f !== 'none') el.setAttribute('fill', ink);
      const st = el.getAttribute('stroke'); if (st && st !== 'none') el.setAttribute('stroke', ink);
    });
    const xml = new XMLSerializer().serializeToString(c);
    const img = new Image();
    await new Promise((res, rej) => { img.onload = res; img.onerror = () => rej(new Error('แปลงภาพโน้ตสากลไม่สำเร็จ'));
...
export function makePainter(canvas, L, pal, meta, fonts = {}) {
  ...
  function drawNotes(cur, t) {
    ...
    const curLine = curKey != null ? (L.staff ? (staffCursorRect(L, cur)?.line.i ?? -1) : (L.index.get(curKey) ?? -1)) : -1;
    ...
    if (L.staff) { drawStaffLines(cur, curLine, vis0, vis1); g.restore(); drawFooter(t); lastT = t; return; }
    for (const ln of L.lines) {
  ...
  // โน้ตสากล: วาดภาพบรรทัด + แถบครอบห้องที่กำลังเล่น + เส้นวิ่ง
  function drawStaffLines(cur, curLine, vis0, vis1) {
    const hit = staffCursorRect(L, cur);
    for (const ln of L.lines) {
      if (ln.y + ln.h < vis0 || ln.y > vis1) continue;
      g.textAlign = 'left'; g.fillStyle = pal.muted; g.font = `400 21px ${textFont}`;
      g.fillText(ln.label || '', L.padX, ln.y + 15);
      if (hit && hit.line.i === ln.i) {
        const by = ln.notesY + STAFF_BAND.top * ln.scale, bh = STAFF_BAND.height * ln.scale;
        g.fillStyle = pal.on; roundRect(hit.rect.x, by, hit.rect.w, bh, 6); g.fill();
        if (pal.onOutline) { g.strokeStyle = pal.onOutline; g.lineWidth = 1.5; g.stroke(); }
      }
      if (ln.img) g.drawImage(ln.img, L.padX, ln.notesY, ln.width, ln.imgH);
      if (hit && hit.line.i === ln.i) {
        const by = ln.notesY + STAFF_BAND.top * ln.scale, bh = STAFF_BAND.height * ln.scale;
        const lx = hit.rect.x + 6 * ln.scale + hit.frac * (hit.rect.w - 22 * ln.scale);
        g.fillStyle = pal.gold; g.fillRect(Math.round(lx) - 1, by, 3, bh);
      }
    }
  }
...
export async function recordVideo({ canvas, audioBuf, paint, fps = 30, signal, onProgress, choice, tail = 0.3, avComp = 0.07 }) {
  ...
  /* ★ A/V sync (Pk 2 ก.ย. 69: "เสียงมาก่อนภาพ เยอะจนไม่มืออาชีพ") — 3 ชั้น:
     ๑ captureStream(0) + requestFrame() หลังวาดทุกเฟรม — เวลาเฟรมมาจากจังหวะที่เราสั่งเอง
       (เดิม captureStream(30) ปล่อยให้เบราว์เซอร์เก็บเฟรมเองตามการเปลี่ยนของ canvas เวลาจึงคลาด)
     ๒ รอ recorder เริ่มจริง (onstart) ก่อนตั้งเวลาเริ่มเสียง และวาดหน้าปกซ้ำระหว่างรอ
       ให้แทร็กภาพมีเฟรมตั้งแต่ต้นไฟล์เท่า ๆ กับแทร็กเสียง
     ๓ วาด "ล่วงหน้า" เท่าเวลาที่ใช้วาดเฟรม (วัดจริงแบบค่าเฉลี่ยเคลื่อนที่) — เนื้อหาของเวลา t
       เคยถูกบันทึกที่เวลา t+เวลาวาด ภาพจึงช้ากว่าเสียงคงที่ทั้งไฟล์ (วัดได้ ~0.1-0.3 วิ)      */
  const stream = canvas.captureStream(0);
  const vTrack = stream.getVideoTracks()[0];
  const pushFrame = vTrack && vTrack.requestFrame ? () => { try { vTrack.requestFrame(); } catch (e) {} } : null;
  if (!pushFrame) { stream.getVideoTracks().forEach(tr => { try { tr.stop(); } catch (e) {} }); }
  const stream2 = pushFrame ? stream : canvas.captureStream(fps);   // เบราว์เซอร์เก่าไม่มี requestFrame → แบบเดิม
  dest.stream.getAudioTracks().forEach(tr => stream2.addTrack(tr));
  let rec;
  try {
    rec = new MediaRecorder(stream2, { mimeType: pick.mime, videoBitsPerSecond: 8_000_000, audioBitsPerSecond: 192_000 });
  } catch (e) {
    ac.close().catch(() => {});
  ...
  paint(0); if (pushFrame) pushFrame();
  const started = new Promise(res => { rec.onstart = res; setTimeout(res, 400); });
  rec.start(1000);
  await started;                                       // ให้ตัวอัดพร้อมก่อนค่อยตั้งเวลาเริ่มเสียง
  paint(0); if (pushFrame) pushFrame();
  const t0 = ac.currentTime + 0.25;
  src.start(t0);
  const endAt = dur + tail;
  /* ๔ ชดเชยตัวเข้ารหัสของเบราว์เซอร์: วัดจากไฟล์จริง (ถอดเสียง+ไล่หาเฟรม) เฟรมภาพถูกประทับเวลา
     ช้ากว่าเสียงคงที่ ~0.07-0.09 วิ แม้สั่ง requestFrame เอง — ชดเชยด้วย avComp (ค่าปริยาย 0.07) */
  let paintLag = 1 / fps;                              // ค่าเฉลี่ยเวลาที่ใช้วาด 1 เฟรม (วินาที)
  const paintAt = t => {
    const w0 = performance.now();
    paint(t);
    if (pushFrame) pushFrame();
    paintLag = paintLag * 0.9 + Math.min(0.15, (performance.now() - w0) / 1000 + 0.5 / fps) * 0.1;
  };
  ...
  const cleanup = () => {
    ...
    stream2.getTracks().forEach(tr => { try { tr.stop(); } catch (e) {} });
    ac.close().catch(() => {});
  };
  ...
  await new Promise((resolve) => {
    ...
    const frame = () => {
      ...
      // วาดล่วงหน้าเท่าเวลาที่ใช้วาด — เฟรมจะถูกบันทึก ณ เวลาที่เนื้อหาตรงกับเสียงพอดี
      paintAt(Math.max(0, Math.min(t + paintLag + avComp, endAt)));
