...
export function staffCursorRect(L, cur) {
  if (!cur) return null;
  const li = L.index.get(cur.verseIdx); if (li == null) return null;
  const ln = L.lines[li];
  const vm = ln.verses.find(v => v.verseIdx === cur.verseIdx); if (!vm) return null;
  const gpos = vm.offset + cur.pos;
  const rect = ln.rects.find(r => gpos >= r.start && gpos < r.start + r.size); if (!rect) return null;
...
export function makePainter(canvas, L, pal, meta, fonts = {}) {
  ...
  function drawNotes(cur, t) {
    ...
    const curLine = curKey != null ? (L.staff ? (L.index.get(cur.verseIdx) ?? -1) : (L.index.get(curKey) ?? -1)) : -1;
