...
import { leadFor, padStream, toMeasures, checkBars, vexBarNotes, measureWeight, drawRowTies } from './staff';
...
export const SHEET_VERSION = '2 ก.ย. 69 · r2 (โน้ตสากลใช้ตัวแปลงเดียวกับหน้าเว็บ)';

// เตรียมโมเดล: วรรค → บรรทัด → ห้อง (ผ่าน padStream/toMeasures ของ lib/staff)
//   แยกจากการวาดเพื่อทดสอบด้วยเครื่องได้ (unit-sheetstaff)
export function prepareStaff(verses, { source = 'combined', beat = 'thai', measuresPerLine = 8 } = {}) {
  const prepared = (verses ?? []).map((v, vi) => {
    ...
    return { vi, v, positions, handPos, measures: Math.ceil(positions.length / 4) };
  });
  // จัดวรรคลงบรรทัด (นับเป็นห้องไทยเต็ม 4 ตำแหน่ง) · ท่อนใหม่ขึ้นบรรทัดใหม่
  const lines = []; let cur = null, count = 0, lastSec;
  prepared.forEach(pv => {
    const sec = pv.v.section ?? null;
    if (!cur || sec !== lastSec || (count > 0 && count + pv.measures > measuresPerLine)) {
      cur = { sec, secStart: sec !== lastSec, verses: [] };
      lines.push(cur); count = 0; lastSec = sec;
    }
    cur.verses.push(pv); count += pv.measures;
  });
  // แปลงแต่ละบรรทัดเป็นห้องเต็ม 2/4 — แบบสากลเติมตัวหยุดนำ 1 ตำแหน่ง (ลูกตกไปอยู่หลังเส้นกั้นห้อง)
  ...
  lines.forEach(line => {
    ...
    line.handStream = stream.map((_, i) => (i >= lead && i - lead < handRaw.length) ? handRaw[i - lead] : null);
    line.measures = toMeasures(stream);
    line.wrong = checkBars(line.measures);              // ควรว่างเสมอ — กันไว้ให้เทสต์จับ
    line.weights = line.measures.map(measureWeight);    // ห้องโน้ตแน่นกว้างขึ้น ไม่เบียดเส้นกั้นห้อง
    line.label = 'วรรค ' + line.verses.map(pv =>
...
export function drawStaffPage(VF, canvas, g, opt, layout, pageIdx, pageCount, scale = 2) {
  ...
  page.items.forEach(it => {
    ...
    const unit = (g.contentW - STAFF.CLEF_PAD) / totalWeight;
    ...
    line.measures.forEach((m, mi) => {
      ...
      try {
        ...
        new VF.Formatter().joinVoices([voice]).format([voice], Math.max(20, mw - 26));
