...
import { usePermissions, PremiumLock } from './Gate';
import { printRights } from '../lib/ownership';

// components/PrintSheet.js — หน้า /songs/[id]/print ทั้งหน้า ย้ายมาไว้ที่นี่ (Pk 1 ก.ย. 69)
//   ไฟล์ในโฟลเดอร์ [id] อัปผ่าน GitHub web UI ไม่ได้ จึงให้ page.js เหลือบรรทัดเดียว re-export ไฟล์นี้
//   ★ พิมพ์ได้เฉพาะโน้ตที่ตัวเองบันทึก · โน้ตคนอื่นดูได้อย่างเดียว · ผู้ดูแลพิมพ์ได้ทุกเพลง
export const PRINT_VERSION = '1 ก.ย. 69 · r2 (เจ้าของโน้ตเท่านั้น)';
...
export default function PrintSheet() {
  ...
  const [loaded, setLoaded] = useState(false);     // โหลดโน้ตทางแรกเสร็จแล้ว (ถึงจะว่างก็ตาม)
  ...
  async function loadMelody() {
    let q = supabase.from('song_melody').select('verse_no, combined, submitted_by')
      .eq('song_id', id).eq('approved', true).order('verse_no').limit(600);
    q = instrument === 'ทำนองหลัก'
      ? q.or('instrument.eq.ทำนองหลัก,instrument.is.null')
      : q.eq('instrument', instrument);
    const { data } = await q;
    setVerses(data ?? []); setLoaded(true);
  ...
  if (!song || (verses.length === 0 && !loaded)) return <div style={{padding:'2rem',color:'#333'}}>กำลังโหลด...</div>;
  // ★ ต้องรู้ก่อนว่าโน้ตทางนี้ใครบันทึก (submitted_by) จึงตัดสินได้ — เช็คหลังโหลดโน้ตแล้ว
  const rights = printRights({ userId: user?.id, isAdmin, verses, song, can });
  if (!rights.print) return (
    <main className="container" style={{maxWidth:'520px',paddingTop:'3rem'}} data-print-locked>
      {rights.owner || isAdmin
        ? <PremiumLock feature="ฉบับพิมพ์และการบันทึก PDF" />
        : <div className="card" style={{textAlign:'center',padding:'1.5rem'}}>
            <div style={{fontSize:'1.8rem'}}>🔒</div>
            <div style={{fontWeight:700,margin:'0.5rem 0'}}>โน้ตนี้เปิดดูได้อย่างเดียว</div>
            <div style={{fontSize:'0.82rem',color:'var(--muted)',lineHeight:1.8}}>{rights.reason}</div>
          </div>}
      <a href={`/songs/${id}`}><button className="btn btn-outline btn-sm" style={{marginTop:'0.8rem'}}>← กลับหน้าเพลง</button></a>
    </main>
  );
    ...
    <div className="print-root">
      ...
      <div className="sheet">
        ...
        <div className="sheet-footer">
          ...
          <span>thaimusicarchive.com · {today}<span data-printver style={{display:'none'}}>{PRINT_VERSION}</span></span>
        </div>
      </div>
    </div>
  );
